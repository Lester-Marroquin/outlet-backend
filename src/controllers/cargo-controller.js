...
const crear = async (body) => {
  try {
    ...
    if (validacion) {
      return responseFail({message: 'Los datos no son validos para crear el cargo', statusCode: StatusCodes.BAD_REQUEST})
    }
...
const actualizar = async (body, id) => {
  try {
    ...
    const validacion = validar(body);
    if (validacion) {
      return responseFail({message: 'Los datos no son validos para la actualización de cargo', statusCode: StatusCodes.BAD_REQUEST})
    }
