...
const { validar } = require('../schemas/persona-schema');
const query  = require('../querys/persona-query');
...
const crear = async (body) => {
  try {
    ...
    if (validacion) {
      return responseFail({data: validacion.details[0].message, message: 'Los datos no son validos para crear una persona' , statusCode: StatusCodes.BAD_REQUEST})
    }
    ...
    return responseSuccess({data: result, message: 'Persona creado exitosamente'});
...
const actualizar = async (body, id) => {
  try {
    ...
    const validacion = validar(body);
    if (validacion) {
      return responseFail({data: validacion.details[0].message, message: 'Los datos no son validos para actualizar la persona', statusCode: StatusCodes.BAD_REQUEST})
    }
    ...
    return responseSuccess({data: result, message: 'Persona actualizado exitosamente'});
