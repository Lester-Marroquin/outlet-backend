...
const { validar } = require('../schemas/imagen-producto-schema');
const query  = require('../querys/imagen-producto-query');
...
const crear = async (body) => {
  try {
    ...
    if (consulta) {
      return responseFail({message: `La imagen del producto ya se encuentra creado con el ID: ${consulta.CodImagenProducto}`, statusCode: StatusCodes.CONFLICT})
    }
    ...
    return responseSuccess({data: result, message: 'Imagen del producto creada exitosamente'});
...
const actualizar = async (body, id) => {
  try {
    ...
    return responseSuccess({data: result, message: 'Imagen del producto actualizada exitosamente'});
