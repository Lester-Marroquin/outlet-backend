...
const crear = async (body) => {
  try {
    ...
    if (validacion) {
      return responseFail({data: validacion.details[0].message, message: 'Los datos no son validos para crear un Emplado', statusCode: StatusCodes.BAD_REQUEST})
    }
    ...
    if (!result) {
      return responseFail({message: 'Error en la creación de Empleado', statusCode: StatusCodes.NOT_FOUND})
    }

    //Destructruación de Empleado para retornar datos sin Clave
    const { ClaveEmpleado, ...dataResult} = result

    return responseSuccess({data: dataResult, message: 'Empleado creado exitosamente'});
...
const actualizar = async (body, id) => {
  try {
    ...
    return responseSuccess({data: dataResult, message: 'Empleado actualizado exitosamente'});
